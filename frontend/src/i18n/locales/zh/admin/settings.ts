    ...
    settings: {
      ...
      features: {
        ...
        webChat: {
          ...
          defaultModelHint: '选项来自下方模型表中类型为“对话”且未勾选“仅限 API”的模型；也可以直接输入自定义模型 ID。',
          modelsTitle: '模型配置',
          modelsHint: '配置通过网页聊天暴露给用户的模型。模型候选来自全部分组的可用模型，也支持输入自定义 ID。',
          addModel: '添加模型',
          empty: '暂未配置模型，点击“添加模型”开始。',
          modelPlaceholder: '选择或输入模型 ID',
          displayNamePlaceholder: '展示名',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 MUC AI 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 MUC AI 用户登录',
      ...
      defaults: {
        ...
        studentVerificationRewardAmountUnit: '单位：CNY（人民币钱包记账单位）',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，MUC AI 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游 MUC AI 站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      payment: {
        ...
        maxAmount: '最高金额（CNY）',
        dailyLimit: '每日限额（CNY）',
        balanceRechargeMultiplier: '余额充值倍率（固定 1）',
        balanceRechargeMultiplierHint: '钱包按人民币记账，充值固定按 1:1 入账；美元支付按 1 USD = 6.7 CNY 折算。',
        balanceRechargePreview: '固定：1 CNY = 1 CNY',
        usdToCnyDisplayRate: 'USD/CNY 展示换算（固定 6.7）',
        usdToCnyDisplayRateHint:
          '固定 1 USD = 6.7 CNY，用于一键切换 USD 计量成本与人民币展示；钱包余额及套餐价格已按 CNY 记账。',
        usdToCnyDisplayRateDisabled: '未启用，显示美元',
        subscriptionUsdToCnyRate: '订阅 CNY 换算汇率',
        subscriptionUsdToCnyRateHint:
          '此设置已停用。套餐统一按 CNY 定价与记账，请保持为 0。',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 MUC AI 订单中，上游 type 会作为易支付 type 参数提交。',
