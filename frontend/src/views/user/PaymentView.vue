  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <template v-else>
        ...
        <!-- Select phase content：/purchase 仅保留充值（订阅购买统一在 /pricing） -->
        <template v-else>
          <!-- 余额充值被后台关闭时提示不可用 -->
          <div v-if="checkout.balance_disabled" class="card py-16 text-center">
          ...
          <template v-else>
            ...
            <div class="card p-5">
              ...
              <p class="mt-0.5 text-sm font-medium text-green-600 dark:text-green-400">{{ t('payment.currentBalance') }}: {{ currencyStore.formatCNY(user?.balance || 0) }}</p>
            </div>
            ...
            <div v-if="validAmount > 0" class="card p-6">
              <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ currencyStore.formatCNY(creditedAmount) }}</span>
                </div>
              </div>
            </div>
...
const creditedAmount = computed(() => {
  const conversion = selectedCurrency.value === 'USD' ? 6.7 : 1
  return Math.round(validAmount.value * conversion * 100) / 100
})
...
onMounted(async () => {
  try {
    ...
    const hadResumeQuery = hasWechatResumeQuery(route.query)
    ...
    // 套餐购买已统一收敛到 /pricing；历史订阅购买链接（?tab=subscription…）明确重定向
    if (route.query.tab === 'subscription') {
      await router.replace('/pricing')
      return
    }
    // 仅订阅站点没有充值能力：统一进 /pricing（微信恢复链不受影响——其查询参数在上面已被剥离）
    if (!hadResumeQuery && resolveSiteBillingMode(appStore.cachedPublicSettings) === 'subscription_only') {
      await router.replace('/pricing')
      return
    }
