        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ currencyStore.formatCNY(row.refund_amount) }}</span>
            ...
            <table class="w-full text-left text-sm">
              ...
              <tbody>
                <tr v-for="row in planChangeRows" :key="row.ID" class="border-b border-gray-50 dark:border-dark-700/50">
                  ...
                  <td class="py-2 pr-4">{{ row.ChangeType === 'upgrade' ? currencyStore.formatCNY(row.AmountDue) : '—' }}</td>
                  <td class="py-2 pr-4">
                    <span
        ...
        <div class="card p-4 space-y-3">
          ...
          <ul v-if="ledgerRows.length" class="space-y-1.5 text-sm">
            <li v-for="(row, i) in ledgerRows" :key="i" class="flex items-center justify-between rounded-xl border border-gray-100 px-3 py-2 dark:border-dark-700">
              <span class="truncate font-mono text-xs">{{ t('wallet.ledgerType.' + row.type) }} · {{ currencyStore.formatCNY(row.amount) }}</span>
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ currencyStore.formatCNY(selectedOrder.amount) }}</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ currencyStore.formatCNY(selectedOrder.refund_amount) }}</p></div>
