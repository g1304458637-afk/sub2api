...
export async function list(
  ...
  filters?: {
    ...
    education_email_verified?: 'true' | 'false'  // filter users by campus-email verification status
  ...
  const params: Record<string, any> = {
    ...
    education_email_verified: filters?.education_email_verified,
...
export interface RevokeEducationEmailResponse {
  user_id: number
  revoked_count: number
}

/** Revoke every campus-email verification identity of one user. */
export async function revokeEducationEmail(userId: number): Promise<RevokeEducationEmailResponse> {
  const { data } = await apiClient.delete<RevokeEducationEmailResponse>(
    `/admin/users/${userId}/education-email`
  )
  return data
}
