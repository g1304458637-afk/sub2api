...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready MUC AI setups.
  ...
  const configContent = `# Grok Build CLI → MUC AI Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for MUC AI Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches MUC AI).
...
description = "Grok 4.5 via MUC AI (Responses)"
...
# Imagine tools: model IDs go to MUC AI media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set MUC_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:MUC_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export MUC_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → MUC AI Grok group
...
# Grok/MUC AI path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:MUC_API_KEY="${apiKey}"`
    : `export MUC_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> MUC AI ${label} group
model_provider = "muc"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at MUC AI OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via MUC AI'
