  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string>) => {
        if (key === 'profile.authBindings.title') return 'Sign-in email'
        if (key === 'profile.authBindings.description') return 'View your sign-in email status'
...
describe('ProfileIdentityBindingsSection', () => {
  ...
  it('renders only the email binding entry for a campus account', () => {
