      ...
      <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-gray-200/60 pt-2.5 dark:border-dark-600/60">
          ...
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 dark:text-dark-400">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-emerald-600 dark:text-emerald-400">{{ currencyStore.formatCNY(totalRecharged) }}</span>
          </p>
        </div>
      </div>
...
const typeOptions = computed(() => [
  ...
  { value: 'reward_grant', label: t('admin.users.typeRewardGrant') },
...
const isBalanceType = (type: string) =>
  type === 'balance' || type === 'admin_balance' || type === 'affiliate_balance' || type === 'reward_grant'
...
const getItemTitle = (item: BalanceHistoryItem) => {
  switch (item.type) {
    ...
    case 'reward_grant':
      return t('redeem.balanceAddedReward')
    case 'concurrency':
...
const formatValue = (item: BalanceHistoryItem) => {
  if (isBalanceType(item.type)) {
    ...
    const amount = item.currency?.toUpperCase() === 'USD'
      ? currencyStore.formatUSD(item.value)
      : currencyStore.formatCNY(item.value)
    return sign + amount
  }
