...
const apiKeyAuthSnapshotVersion = 25 // v25: subscription quota policy and 5-hour limit
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			QuotaPolicy:                     apiKey.Group.QuotaPolicy,
			ShortLimitUSD:                   apiKey.Group.ShortLimitUSD,
			...
			MusicPricePerTrack:              apiKey.Group.MusicPricePerTrack,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			QuotaPolicy:                     snapshot.Group.QuotaPolicy,
			ShortLimitUSD:                   snapshot.Group.ShortLimitUSD,
			...
			MusicPricePerTrack:              snapshot.Group.MusicPricePerTrack,
