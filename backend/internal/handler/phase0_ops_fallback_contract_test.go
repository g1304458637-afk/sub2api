...
//  3. :1018 `currentSubscription = nil` + :1020 `retryWithFallback = true`
...
func TestPhase0OpsFallbackNilSubscriptionPrecedent(t *testing.T) {
	...
	for i, line := range lines {
		n := i + 1
		switch {
		case strings.Contains(line, "currentSubscription = nil"):
	...
	require.Greater(t, nilAssignLine, 0, "ops fallback precedent (`currentSubscription = nil`) must exist")
