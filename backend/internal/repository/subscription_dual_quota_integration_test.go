...
func TestDualQuotaSettlementResetAndDedup(t *testing.T) {
	...
	// PostgreSQL timestamps have microsecond precision; keep the replayed reset
	// event aligned to that precision so repeated EffectiveAt compares exactly.
	at := time.Now().Add(time.Millisecond).Truncate(time.Microsecond)
